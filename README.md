...
- 分块级符号条件：
  - `phoneme_id`: `[B, T]`
  - `cond_num`: `[B, T, 7]`, features are
  - `cond_num`: `[B, T, 7]`，特征包括
    - `[pitch, note_on, note_off, t_since_on, t_to_end, note_prog, ph_prog]`
...
- `pitch` scalar -> 2-layer MLP (`pitch_proj_dim`)
- `pitch` 标量 -> 两层 MLP（`pitch_proj_dim`）
- Remaining 6 timing/progress scalars -> 2-layer MLP (`time_proj_dim`)
- 其余 6 个时序/进度标量 -> 两层 MLP（`time_proj_dim`）
